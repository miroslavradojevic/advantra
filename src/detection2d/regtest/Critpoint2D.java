...
import detection2d.Delineator2D;
import detection2d.PeakExtractor2D;
import ij.*;
import ij.gui.*;
...
//        float _detection_sensitivity; // ranges from 0 (max) - 1 (max. entropy) will be used at thresholding
        ...
        float   _reg_radius = 3f;        // defines the size of the neighbourhood
        int     _reg_type   = RankFilters.MEAN; // defines the regularisation type
        ...
        if (Macro.getOptions()==null) {
            ...
            _reg_radius             = (float)   Prefs.get("critpoint.detection2d.reg_radius", 3f);
            _reg_type               = (int)     Prefs.get("critpoint.detection2d.reg_type", RankFilters.MEAN);
            ...
            gd.addNumericField("reg_radius",        _reg_radius,        1, 5, "");
            gd.addNumericField("reg_type",          _reg_type,          0, 5, "(0-MEAN, 4-MEDIAN)");
            ...
            _reg_radius     = (float) gd.getNextNumber();           Prefs.set("critpoint.detection2d.reg_radius", _reg_radius);
            _reg_type       = (int)   gd.getNextNumber();           Prefs.set("critpoint.detection2d.reg_type", _reg_type);
...
//            System.out.println(Macro.getOptions());
            ...
            _reg_radius         = Float.valueOf(Macro.getValue(Macro.getOptions(),      "reg_radius", String.valueOf(3)));
            _reg_type           = Integer.valueOf(Macro.getValue(Macro.getOptions(),    "reg_type", String.valueOf(RankFilters.MEAN)));
...
//        if (true) {
//            System.out.println("----------");
//            System.out.println(_reg_radius);
//            System.out.println("regularisation: " + ((_reg_type==RankFilters.MEAN)? "MEAN" : (_reg_type==RankFilters.MEDIAN)? "MEDIAN" : "NOTHING") );
//            System.out.println("----------");
//            System.out.println("closing...");
//            return;
//        }

        if (!(_reg_type==RankFilters.MEAN || _reg_type==RankFilters.MEDIAN)) {
            System.out.println("regularisation type wrong, exiting...");
            return;
        }
