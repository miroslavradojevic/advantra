...
public class NeuronStalker implements PlugIn {
    ...
    public void run(String s) {
        ...
        percentile              = (float)   Prefs.get("neuronstalker.percentile", 90f);
        correlation_boundary    = (float)   Prefs.get("neuronstalker.correlation_boundary", 0.75f);
        std_angle_deg           = (float)   Prefs.get("neuronstalker.std_angle_deg", 60f);
        ...
        zDist = (float) Prefs.get("neuronstalker2d.zDist", 2f);

        search_mode             = (int)     Prefs.get("neuronstalker.search_mode", 0);
...
//        if(true)return;
        ...
        File swc_dir = new File(inimg_dir_path + String.format("NeuronStalker.D_"+"%.1f", neuron_diameter));
