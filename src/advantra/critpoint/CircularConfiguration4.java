 ...
 */
public class CircularConfiguration4 {

	public int			r;
	public int			d;

	public static int			nPeaks = 4;
	...
	)
	{
		...
		// make combinations of R and T (thickness) with the angle corresponding to T
		for (int Rpix = r; Rpix >= (int)(0.5*r); Rpix*= 0.75) {
			...
			for (float ratio = 0.5f; ratio >= 0.1; ratio*=0.75){
				...
				if (Tpix>=2){
					...
					comb.add(new int[]{Rpix, Tpix, angRes}); System.out.println("4 : "+Rpix+","+Tpix+","+angRes);
				}
				else {
					break;
				}
		...
		for (int combIdx = 0; combIdx < comb.size(); combIdx++) {
			...
			//System.out.println("4 reaad : "+Tpx);
			...
			for (int d1 = startscale*minAngScaleDegrees; d1 < 360; d1+=minAngScaleDegrees){
				for (int d2 = startscale*minAngScaleDegrees; d2 < 360; d2+=minAngScaleDegrees){
					for (int d3 = startscale*minAngScaleDegrees; d3 < 360; d3+=minAngScaleDegrees){
						for (int d4 = startscale*minAngScaleDegrees; d4 < 360; d4+=minAngScaleDegrees){

							boolean isConfiguration = (d1+d2+d3+d4==360) && (d1<=180) && (d2<=180) && (d3<=180) && (d4<=180) &&
															  (d1+d2)>=160 && (d1+d2)<=200 && (d2+d3)>=160 && (d2+d3)<=200;
							...
							if(isConfiguration) {
								...
								if(!covered){
									...
									//System.out.println("4 adding : "+Tpx);
									comb1.add(new int[]{d1, d2, d3, d4, Rpx, Tpx});
									float[] inhere = new float[nPeaks];
									inhere[0] = (d1/360f)*TwoPi;
									inhere[1] = (d2/360f)*TwoPi;
									inhere[2] = (d3/360f)*TwoPi;
									inhere[3] = (d4/360f)*TwoPi;
									String name = ""+d1+","+d2+","+d3+","+d4+":"+Rpx+","+Tpx;
									names.add(name);
									//System.out.println("4 forming : "+name);
...
//							else {
//								System.out.println("was not configuration: "+d1+","+d2+","+d3+","+d4);
//							}
						}
					}
