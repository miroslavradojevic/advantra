...
public void interactAndOptimize (
	...
	energy = null;
	if (display == null) {
		// don't stop the execution because there is no image
		// actually it will be necessary to set display to null
		// in order for the rest of the code to work
		//return;
	}
	...
	startOptimizing(); // to enable oprimization label
	//this.display = null; // to avoid graphic interface interaction
...
//		X[k] = new Snake2DNode(youngSnake[k].x, youngSnake[k].y,
//			youngSnake[k].frozen, youngSnake[k].hidden);
		System.out.println("node["+k+"]="+X[k].getX()+", "+X[k].getY());
...
public void justOptimize ( // IT WILL SUBSTITUTE interactAndOptimize()
		final Snake2D snake,
		final double[] nodes
	) {
		this.snake = snake;
		...
		if (snake == null) {
			System.out.println("There was no snake! Returning...");
			return;
		}
		...
		startOptimizing(); // to enable optimization label
		optimize(X);
		
		if(X.length!=2){
			System.out.println("There should be 2 nodes after optimization.");
			return;
		}
...
private void optimize (
	...
	if (G0 == null) {
		System.out.println("g(X) was null! NO OPTIMIZATION!");
		return;
	}
...
public void actionPerformed (
	...
	if (ae.getActionCommand().equals("Stamp")) {
		...
		ip.setColor(PolygonRoi.getColor());
		int m = 0;
		int M = 0;
		int s = 0;
		int S = 0;
