...
public class GaborFilt2D_OLD extends Thread { 
	
	/*
	 * this one has filter calculated each time a new patch is convolved with an image in a run() method
	 * it is more effective to precalculate all the patches to avoid allocations 
	 * inside the for() loop. However that doesn't speed up that much, similar amount of computation still has to be done
	 * 
	 * Main reason for abandoning it in favour of processing scales in parallel
	 * will be because of the patch calculation in parallel
	 * - it is slower 
	 * - accuracy: patch borders are losing their authentic values
	 *   
	 */
	...
	*/
	...
	/*
	 * image split in patches
	 * patches will be processed in parallel
	 */
	...
	public GaborFilt2D_OLD(int n0, int n1){
		this.n0 = n0;
		this.n1 = n1;
	}
	...
	public static void load(
		...
		// split into patches ptch_len x ptch_len, easier wrapped in imagescience
