...
public class MySnakuscule implements Snake2D
...
//node[0] = new Snake2DNode((int)(0.5 * (double)width * (1-DIST_FROM_CENTER)), height / 2);
node[0] = new Snake2DNode(2, height / 2);
//node[1] = new Snake2DNode((int)(0.5 * (double)width * (1+DIST_FROM_CENTER)), height / 2);
node[1] = new Snake2DNode(width-1, height / 2);

System.out.println("snake started with wxh="+width+"x"+height);

System.out.println("node[0]: "+node[0].getX()+", "+node[0].getY());
System.out.println("node[1]: "+node[1].getX()+", "+node[1].getY());
