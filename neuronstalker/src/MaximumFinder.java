...
public class MaximumFinder implements ExtendedPlugInFilter, DialogListener {
    ...
    public void run(ImageProcessor ip) {
        Roi roi = imp.getRoi();
        outputType = IN_TOLERANCE; // to segment soma
    ...
    //***
    // soma extraction using maximum finder - the aim is to do binary segmentation
    // the output is a binary image that segments the soma out
    public ImagePlus extractMaxWithTolerance(ImageProcessor ip, float input_tolerance) {

        Roi roi = imp.getRoi();
        outputType = IN_TOLERANCE; // to segment soma

        if (input_tolerance<0)
            tolerance = 0;
        else
...
//        }
        ByteProcessor outIp = null;
//        System.out.println("do it with tolerance = " + tolerance);
        outIp = findMaxima(ip, tolerance, threshold, outputType, excludeOnEdges, false); //process the image
        if (outIp == null) return null;              //cancelled by user or previewing or no output image
        if (!Prefs.blackBackground)             //normally, output has an inverted LUT, "active" pixels black (255) - like a mask
            outIp.invertLut();
//        String resultName;
//        if (outputType == SEGMENTED)            //Segmentation required
...
//        if (WindowManager.getImage(outname)!=null)
//            outname = WindowManager.getUniqueName(outname);
        ImagePlus maxImp = new ImagePlus("", outIp);
        Calibration cal = imp.getCalibration().copy();
        cal.disableDensityCalibration();
        maxImp.setCalibration(cal);             //keep the spatial calibration
        return maxImp;
